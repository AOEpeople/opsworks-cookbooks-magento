...
if node.key?('additional-elbs') && node['additional-elbs'].key?('elbs')
  node['additional-elbs']['elbs'].each do |elb|
    ...
    execute "Register instance #{currentInstanceId} with ELB #{elb}" do
      command "aws --region #{node[:opsworks][:instance][:region]} elb deregister-instances-from-load-balancer --load-balancer-name #{elb} --instances \"#{node[:opsworks][:instance][:aws_instance_id]}\""
      environment(
